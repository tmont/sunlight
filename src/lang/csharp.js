var utils = require('../util'),
	...
	primitives = ['int', 'bool', 'double', 'float', 'char', 'byte', 'sbyte', 'uint', 'long', 'ulong', 'char', 'decimal', 'short', 'ushort'],
...
module.exports = {
	...
	customParseRules: [
		...
		function(context) {
			...
			rule = utils.createProceduralRule(context.count() - 1, -1, [
			...
			//now we need to look ahead and verify that the next non-utils.whitespace token is '{' or ';'
			count = 'get'.length;
	...
	namedIdentRules: {
		custom: [
			...
			function(context) {
				...
				nextToken = utils.getNextNonWsToken(context.tokens, context.index);
			...
			createNamedIdentFunction(function(context) {
				var token = utils.getNextNonWsToken(context.tokens, context.index),
					index,
					bracketCount,
					foundComma = false,
					indexOfLastBracket;
				...
				token = utils.getNextNonWsToken(context.tokens, indexOfLastBracket);
			...
			function() {
				var precedes = [
					[utils.whitespace, { token: 'punctuation', values: [')'] }, utils.whitespace, { token: 'ident' }],
					[utils.whitespace, { token: 'punctuation', values: [')'] }, utils.whitespace, { token: 'keyword', values: ['this'] }]
				];
				...
				return createNamedIdentFunction(function(context) {
					...
					while (token = context.tokens[--index]) {
						if (token.name === 'punctuation' && token.value === '(') {
							prevToken = utils.getPreviousNonWsToken(context.tokens, index);
			...
			function(context) {
				...
				while (token = context.tokens[--index]) {
					if (token.name !== 'ident' && token.name !== 'default' && (token.name !== 'operator' || token.value !== '.')) {
						...
						return utils.createProceduralRule(
							index - 1, 
							-1, 
							[{ token: 'keyword', values: ['using'] }, { token: 'default' }, { token: 'ident' }, utils.whitespace]
						)(context.tokens);
					}
				}
			...
			createNamedIdentFunction(function(context) {
				var i,
					follows = [
						...
						[{ token: 'keyword', values: ['class', 'interface', 'event', 'struct', 'enum', 'delegate', 'public', 'private', 'protected', 'internal', 'static', 'virtual', 'sealed', 'params'] }, utils.whitespace],

						//typeof/default
						[{ token: 'keyword', values: ['typeof', 'default'] }, utils.whitespace, { token: 'punctuation', values: ['('] }, utils.whitespace ],
						
						//casting using 'as'
						[{ token: 'keyword', values: ['as'] }, utils.whitespace ]
					],
					precedes = [
						//arrays
						[utils.whitespace, { token: 'punctuation', values: ['['] }, utils.whitespace, { token: 'punctuation', values: [']'] }], //in method parameters
				...
				for (i = 0; i < follows.length; i++) {
					if (utils.createProceduralRule(context.index - 1, -1, follows[i], false)(context.tokens)) {
						return true;
					}
				}
				
				for (i = 0; i < precedes.length; i++) {
					if (utils.createProceduralRule(context.index + 1, 1, precedes[i], false)(context.tokens)) {
						return true;
					}
				}
